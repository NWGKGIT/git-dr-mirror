...
if command -v uv >/dev/null 2>&1; then
    ...
    command -v python3 >/dev/null 2>&1 \
        || die "python3 is not installed. Install Python 3.13+ or uv (https://docs.astral.sh/uv/)."
    python3 -c 'import sys; sys.exit(0 if sys.version_info >= (3, 13) else 1)' \
        || die "Python 3.13+ required, found $(python3 --version 2>&1). Install a newer Python or uv (https://docs.astral.sh/uv/ — uv can provision Python for you)."
